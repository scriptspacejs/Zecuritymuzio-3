...
// Free AI Chat functionality using Hugging Face Inference API
async function generateAIResponse(message) {
    try {
        // Use Hugging Face's free inference API (no API key required for basic models)
        ...
        if (!response.ok) {
            ...
            if (fallbackResponse.ok) {
                ...
                return data[0]?.generated_text?.replace(message, '').trim() || generateFallbackResponse(message);
            }
            return generateFallbackResponse(message);
        }

        const data = await response.json();
        return data[0]?.generated_text || generateFallbackResponse(message);
    } catch (error) {
        log("error", `AI response error: ${error.message}`);
        return generateFallbackResponse(message);
    }
}
...
function generateFallbackResponse(message) {
    const responses = [
        ...
        "That reminds me of something similar I've heard before.",
        "What made you think of that?",
        "That's quite thoughtful! I appreciate you sharing.",
        "I find that really interesting. What else can you tell me?"
    ];
...
const aiChatEnabled = new Map(); // guildId: boolean
...
async function handleAiChatCommand(interaction) {
    try {
        ...
        aiChatEnabled.set(interaction.guild.id, enable);

        // Save to database
        await saveServerConfiguration(interaction.guild.id, { aiChatEnabled: enable });

        const embed = new EmbedBuilder()
            .setTitle(`🤖 AI Chat ${enable ? 'Enabled' : 'Disabled'}`)
            .setDescription(`AI chat responses have been ${enable ? 'enabled' : 'disabled'} for this server.`)
            .setColor(enable ? 0x00ff00 : 0xff0000)
            .addFields(
                { name: "🎯 Status", value: enable ? "✅ Active" : "❌ Disabled", inline: true },
                { name: "🌐 Scope", value: "All channels in server", inline: true },
                { name: "🔧 Features", value: enable ? "• Free AI responses\n• Natural conversations\n• Automatic replies\n• No API keys needed" : "AI chat disabled", inline: false },
                { name: "💡 How it works", value: enable ? "The bot will respond to regular messages (not commands) with AI-generated replies using free Hugging Face models." : "Bot will not respond to regular messages.", inline: false }
            )
            .setTimestamp();
        ...
        await securityBot.logSecurityEvent(
            ...
            interaction.user,
            `AI chat ${enable ? 'enabled' : 'disabled'}`
        );

    } catch (error) {
        log("error", `AI chat command error: ${error.message}`);
        const embed = new EmbedBuilder()
            .setTitle("❌ AI Chat Configuration Failed")
            .setDescription(`Failed to configure AI chat: ${error.message}`)
            .setColor(0xff0000);
        await interaction.reply({ embeds: [embed], ephemeral: true });
    }
}
            ...
            aiChatEnabled.set(guildId, config.aiChatEnabled !== 0); // Default to enabled
...
async function saveServerConfiguration(guildId, updateData = {}) {
    try {
        const config = {
            ...
            aiChatEnabled: aiChatEnabled.get(guildId) !== false, // Default to enabled
...
client.on('messageCreate', async (message) => {
    ...
    // AI Chat Response System - responds to all non-command messages
    if (!messageContent.startsWith('/') && 
        !messageContent.startsWith('ksb ') && 
        !messageContent.startsWith('!') &&
        !messageContent.startsWith('play ') &&
        message.content.length > 0 &&
        aiChatEnabled.get(message.guild.id) !== false) { // AI chat is enabled by default
        ...
        try {
            ...
            if (aiResponse && aiResponse.trim().length > 0) {
                // Send AI response with a small delay to feel more natural
                setTimeout(async () => {
                    try {
                        await message.reply(aiResponse);
                        log("info", `AI responded to ${message.author.tag}: "${aiResponse}"`);
                    } catch (replyError) {
                        log("error", `Failed to send AI reply: ${replyError.message}`);
                    }
...
async function registerSlashCommands() {
    ...
    const commands = [
        ...
        new SlashCommandBuilder()
            .setName('ai_chat')
            .setDescription('🤖 Configure AI chat responses')
            .addBooleanOption(option =>
                option.setName('enable')
                    .setDescription('Enable or disable AI chat responses')
                    .setRequired(true)
            )
    ];
